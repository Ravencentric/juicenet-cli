...
class Resume:
    ...
    def log_file_info(self, file: Path) -> None:
        ...
        if not self.disable:
            ...
            csv.DictWriter(self.path.open("a"), fieldnames=["name", "size", "count", "scope"], quoting=csv.QUOTE_ALL).writerow(info)
    ...
    def filter_uploaded_files(self, files: list[Path]) -> list[Path]:
        ...
        if not self.disable:
            not_uploaded = []
            resume_data = tuple(csv.DictReader(self.path.open("r"), fieldnames=["name", "size", "count", "scope"], quoting=csv.QUOTE_ALL))
